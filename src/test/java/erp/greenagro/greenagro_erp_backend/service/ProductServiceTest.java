...
class ProductServiceTest {
    ...
    /**
        ...
        3. partner, productGroup 요청 없을때도 가능
        4. partner, productGroup 요청은 있는데 조회시 없는경우 -> 오류
    ...
    void 품목_생성_정상(){
        //given
        Partner partner = partnerRepository.save(getPartner());
        ProductGroup group = productGroupRepository.save(new ProductGroup("테스트 그룹"));
        CreateProductRequest request = getCreateProductRequest("001","테스트품목", group, partner);
        ...
        assertEquals(partner, product.getPartner()); //jpa 영속성
        assertEquals(request.getPartnerId(), product.getPartner().getId());
    ...
    void 품목_생성_이름_코드_중복(){
        ...
        CreateProductRequest request = getCreateProductRequest("P001","상품A", group, partner);
        CreateProductRequest requestDuplicateCode = getCreateProductRequest("P001","상품B", group, partner);
        CreateProductRequest requestDuplicateName = getCreateProductRequest("P002","상품A", group, partner);
        CreateProductRequest requestDuplicateAll = getCreateProductRequest("P001","상품A", group, partner);
    ...
    void 품목_생성_회사_그룹_null_가능(){
        ...
        //when-then 정상 저장 (Partner, ProductGroup 필수 아님)
        productService.createProduct(request);
    }
    ...
    void 품목_수정_정상(){
        ...
        Partner partner = partnerRepository.save(getPartner());
        ProductGroup group = productGroupRepository.save(new ProductGroup("테스트 그룹"));
        Product beforeProduct = getProductAndSave("P001", "품목A", group, partner); //나중에 null일때도 테스트

        Partner partner2 = partnerRepository.save(getPartner2());
        ProductGroup group2 = productGroupRepository.save(new ProductGroup("테스트 그룹2"));
        UpdateProductRequest request = getUpdateProductRequest("P002", "품목B", group2, partner2);
        ...
        assertEquals(request.getPartnerId(), afterProduct.getPartner() != null ? afterProduct.getPartner().getId() : null);
    ...
    void 품목_수정_코드_이름_중복(){
        ...
        Product existingProduct = getProductAndSave("P001", "품목A", group, partner); //나중에 null일때도 테스트

        Product targetProduct = getProductAndSave("P002", "품목B", group, partner); //나중에 null일때도 테스트

        UpdateProductRequest requestDuplicateCode = getUpdateProductRequest("P001", "품목B", group, partner);
        UpdateProductRequest requestDuplicateName = getUpdateProductRequest("P002", "품목A", group, partner);
        UpdateProductRequest requestDuplicateAll = getUpdateProductRequest("P001", "품목A", group, partner);
    ...
    void 품목_수정_회사_그룹_null_가능(){
        ...
        assertNull(afterProduct.getPartner());
    }
    ...
    void 품목_전체_조회(){
        ...
        List<Product> savedProducts = List.of(
                productRepository.save(getProductAndSave("001","품목1", group, partner)),
                productRepository.save(getProductAndSave("002","품목2", group, partner)),
                productRepository.save(getProductAndSave("003","품목3", group, partner))
        );
        ...
        for (int i = 0; i < savedProducts.size(); i++) {
            ...
            assertEquals(expected.getPartner().getId(), actual.getPartner().getId());
            assertEquals(expected.getPartner().getBizName(), actual.getPartner().getName());
    ...
    void 품목_상세_조회(){
        ...
        Partner partner = partnerRepository.save(getPartner());
        ProductGroup group = productGroupRepository.save(new ProductGroup("테스트 그룹"));
        Product product = getProductAndSave("001", "name", group, partner); //나중에 null일때도 테스트
        ...
        assertEquals(product.getPartner().getId(), response.getPartner().getId());
        assertEquals(product.getPartner().getBizName(), response.getPartner().getName());
    ...
    private Partner getPartner() {
        return new Partner(
                PartnerType.CORPORATION, // 사업자 유형
    ...
    //Partner 생성
    private Partner getPartner2() {
        return new Partner(
                PartnerType.SOLE_PROPRIETOR, // 사업자 유형
                ...
                "김우리-2",                   // ourManager
                "이거래-2",                   // PartnerManager
                "테스트 메모-2"
        );
    }


    private CreateProductRequest getCreateProductRequest(String code, String name,ProductGroup group, Partner partner) {
        return new CreateProductRequest(
                ...
                partner != null ? partner.getId() : null,                // PartnerId
    ...
    private UpdateProductRequest getUpdateProductRequest(String code, String name, ProductGroup group, Partner partner) {
    ...
    private Product getProductAndSave(String code, String name, ProductGroup group, Partner partner) {
